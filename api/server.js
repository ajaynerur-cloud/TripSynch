...
if (!GITHUB_OWNER || !GITHUB_REPO || !GITHUB_TOKEN) {
  console.warn("Missing required GitHub environment variables.");
}
...
const makeId = (bytes = 12) =>
  crypto.randomBytes(bytes).toString("base64url");

const hash = value =>
  crypto.createHash("sha256").update(String(value)).digest("hex");

const clean = (value, length = 100) =>
  String(value ?? "").trim().slice(0, length);

async function readStore() {
  const response = await fetch(
    `${contentsUrl}?ref=${encodeURIComponent(GITHUB_BRANCH)}&t=${Date.now()}`,
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `GitHub read failed: ${response.status} ${responseText}`
    );
  }
...
async function writeStore(data, sha, message) {
  ...
  if (!response.ok) {
    ...
    const error = new Error(
      `GitHub write failed: ${response.status} ${responseText}`
    );
  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        error:
          "Valid trip name, owner name, and currency are required"
      });
    }
...
app.post("/api/join", async (req, res, next) => {
  try {
    ...
    if (!code || !name) {
      return res.status(400).json({
        error: "Invite code and member name are required"
      });
    }
...
app.post("/api/trips/:id/expenses", async (req, res, next) => {
  try {
    ...
    await mutateStore(store => {
      ...
      // Important fix:
      // Do not use .map(clean). Array.map passes the array index as
      // clean's second argument, which was incorrectly used as length.
      ...
      const memberIds = new Set(
        trip.members.map(member => clean(member.id, 100))
      );

      const validation = {
        descriptionValid: Boolean(description),
        amountValid: Number.isFinite(amount) && amount > 0,
        paidByValid: memberIds.has(paidBy),
        splitPresent: splitAmong.length > 0,
        splitMembersValid: splitAmong.every(memberId =>
      ...
      if (Object.values(validation).some(value => !value)) {
        ...
        const error = new Error(
          `Invalid expense details: ${Object.entries(validation)
            .filter(([, valid]) => !valid)
            .map(([name]) => name)
            .join(", ")}`
        );
      ...
      trip.expenses.push({
        ...
        paidBy,
        splitAmong,
        createdBy: clean(req.body.actorId, 100),
        createdAt: new Date().toISOString()
      });
...
app.delete("/api/trips/:id", async (req, res, next) => {
  try {
    await mutateStore(store => {
      ...
      if (clean(req.body.actorId, 100) !== trip.ownerId) {
        const error = new Error(
...
app.listen(PORT, () => {
  console.log(`TripSynch API listening on ${PORT}`);
});
